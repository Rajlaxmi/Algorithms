...
pair<node* ,int> construct(int* in, int* post, int s, int e, int posts)//pres needed for starting point in inorder arr
{
	if(s <= e)
	{
		int first = post[posts];
		...
		pair<node* ,int> r = construct(in, post, i+1, e, posts  - 1);
		pair<node* ,int> l = construct(in, post, s, i-1, posts - r.second - 1);
...
int main()
{
	...
	pair<node* ,int> root = construct(in, post, 0, 8, 8);
	preOrder(root.first);
	cout<<endl;
